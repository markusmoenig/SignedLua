...
class Model: NSObject, ObservableObject {
    ...
    func createShapes() {
        shapes = [
            ...
            SignedCommand("Sphere", role: .GeometryAndMaterial, action: .Add, primitive: .Sphere, data: ["Geometry": SignedData([SignedDataEntity("radius", Float(0.35), float2(0, 5))])], material: SignedMaterial(albedo: float3(0.5,0.5,0.5))),
            SignedCommand("Box", role: .GeometryAndMaterial, action: .Add, primitive: .Box, data: ["Geometry": SignedData([SignedDataEntity("size", float3(0.065,0.065,0.065) * 7, float2(0,10), .Slider), SignedDataEntity("rounding", Float(0.0), float2(0,1))])], material: SignedMaterial(albedo: float3(0.5,0.5,0.5))),
            SignedCommand("Cylinder", role: .GeometryAndMaterial, action: .Add, primitive: .Cylinder, data: ["Geometry": SignedData([SignedDataEntity("height", Float(0.015 * 7), float2(0, 5)), SignedDataEntity("radius", Float(0.14), float2(0, 5)), SignedDataEntity("rounding", Float(0), float2(0, 1))])], material: SignedMaterial(albedo: float3(0.5,0.5,0.5))),
        ]
