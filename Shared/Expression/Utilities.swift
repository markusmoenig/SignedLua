...
func extractComponent(_ options: [String:Any], variableName: String = "variable", context: GraphContext, error: inout CompileError) -> UpTo4Data?
{
    var index : Int? = nil
    ...
    if let index = index {
        if let f2 = extractFloat2Value(options, context: context, error: &error, name: variableName, isOptional: true) {
            if index <= 1 {
                var data = UpTo4Data()
                data.data2 = f2
                data.index = index
        ...
        if let f3 = extractFloat3Value(options, context: context, error: &error, name: variableName, isOptional: true) {
            if index <= 2 {
                var data = UpTo4Data()
                data.data3 = f3
                data.index = index
        ...
        if let f4 = extractFloat4Value(options, context: context, error: &error, name: variableName, isOptional: true) {
            if index <= 3 {
                var data = UpTo4Data()
                data.data4 = f4
                data.index = index
...
func extractVariableValue(_ options: [String:Any], variableName: String, context: GraphContext, error: inout CompileError) -> Any?
{
...
func extractFloat4Value(_ options: [String:Any], context: GraphContext, error: inout CompileError, name: String = "float4", isOptional: Bool = false ) -> Float4?
{
    if let value = options[name] as? String {
        ...
        if array.count == 1 {
            if let v = context.getVariableValue(String(array[0])) as? Float4 {
                return v
            }
...
func extractFloat3Value(_ options: [String:Any], context: GraphContext, error: inout CompileError, name: String = "float3", isOptional: Bool = false ) -> Float3?
{
    if let value = options[name] as? String {
        let array = value.split(separator: ",")
        if array.count == 3 {
        ...
        if array.count == 1 {
            if let v = context.getVariableValue(String(array[0])) as? Float3 {
                return v
            }
...
func extractFloat2Value(_ options: [String:Any], context: GraphContext, error: inout CompileError, name: String = "float2", isOptional: Bool = false ) -> Float2?
{
...
func extractFloat1Value(_ options: [String:Any], context: GraphContext, error: inout CompileError, name: String = "float", isOptional: Bool = false ) -> Float1?
{
...
/// Extract a int1 vale
func extractInt1Value(_ options: [String:Any], context: GraphContext, error: inout CompileError, name: String = "int", isOptional: Bool = false ) -> Int1?
{
    if var value = options[name] as? String {
        value = value.trimmingCharacters(in: .whitespaces)
...
func extractBool1Value(_ options: [String:Any], context: GraphContext, error: inout CompileError, name: String = "bool", isOptional: Bool = false ) -> Bool1?
{
    if var value = options[name] as? String {
        value = value.trimmingCharacters(in: .whitespaces)
        ...
        if let v = context.getVariableValue(value) as? Bool1 {
            return v
        }  else { if isOptional == false { error.error = "Parameter '\(name)' not found" } }
    } else { if isOptional == false { error.error = "Parameter '\(name)' not found" } }
    return nil
...
func extractPair(_ options: [String:Any], variableName: String, context: GraphContext, error: inout CompileError, optionalVariables: [String]) -> (UpTo4Data, UpTo4Data,[UpTo4Data])
{
    var Data         = UpTo4Data()
    ...
    if let variableValue = extractVariableValue(options, variableName: variableName, context: context, error: &error) {
        if let b1 = variableValue as? Bool1 {
            ...
            if let data = extractBool1Value(options, context: context, error: &error) {
                Data.bool1 = data
            }
            for oV in optionalVariables {
                var data = UpTo4Data()
                data.bool1 = extractBool1Value(options, context: context, error: &error, name: oV, isOptional: true)
        ...
        if let f3 = variableValue as? Float3 {
            ...
            if let data = extractFloat3Value(options, context: context, error: &error) {
                Data.data3 = data
            }
            for oV in optionalVariables {
                var data = UpTo4Data()
                data.data3 = extractFloat3Value(options, context: context, error: &error, name: oV, isOptional: true)
                optionals.append(data)
            }
        } else
        if let f4 = variableValue as? Float4 {
            variableData.data4 = f4
            if let data = extractFloat4Value(options, context: context, error: &error) {
                Data.data4 = data
            }
            for oV in optionalVariables {
                var data = UpTo4Data()
                data.data4 = extractFloat4Value(options, context: context, error: &error, name: oV, isOptional: true)
                optionals.append(data)
            }
        }
    }
